...
1. `live.mjs`: boot. If the request names or implies a file, route, or app inside a monorepo, infer the concrete path and run `node .grok/skills/impeccable/scripts/live.mjs --target <path>` instead; then run the rest of this live session from the returned `projectRoot`.
...
```bash
node .grok/skills/impeccable/scripts/live.mjs
```
...
```
LOOP:
  node .grok/skills/impeccable/scripts/live-poll.mjs   # default long timeout; no --timeout=
...
```
node .grok/skills/impeccable/scripts/live-poll.mjs --stream   # stays running; one JSON line per event
...
```bash
node .grok/skills/impeccable/scripts/live-status.mjs
node .grok/skills/impeccable/scripts/live-resume.mjs --id SESSION_ID
node .grok/skills/impeccable/scripts/live-complete.mjs --id SESSION_ID
```
...
```bash
node .grok/skills/impeccable/scripts/live-insert.mjs --id EVENT_ID --count EVENT_COUNT --position after \
...
```bash
node .grok/skills/impeccable/scripts/live-poll.mjs --reply EVENT_ID done --file RELATIVE_PATH
```
...
```bash
node .grok/skills/impeccable/scripts/live-poll.mjs --reply EVENT_ID error "Short reason"
```
...
```bash
node .grok/skills/impeccable/scripts/live-poll.mjs --reply EVENT_ID steer_done ["Optional short note for a browser toast"]
```
...
```bash
node .grok/skills/impeccable/scripts/live-server.mjs stop
```
...
```bash
node .grok/skills/impeccable/scripts/detect-csp.mjs
```
